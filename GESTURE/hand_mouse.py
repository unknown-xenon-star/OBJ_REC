...
def move_cursor(index_x: float, index_y: float, *, locked: bool = False) -> None:
    """Move cursor toward target.  When *locked* is True the position is frozen
    (drag is active and the index finger is pressed against the thumb, so raw
    coordinates are unreliable)."""
    global _cursor_x, _cursor_y

    if locked:
        return  # Don't move while dragging — user steers via wrist/whole hand
...
class ClickDragState:
    ...
    def __init__(self) -> None:
        ...
        # Midpoint (normalised) captured when drag starts so the cursor tracks
        # the hand's midpoint rather than the erratic index tip.
        self.drag_anchor_x: float | None = None
        self.drag_anchor_y: float | None = None
    ...
    def update(self, index_tip, thumb_tip) -> None:
        ...
        if self.state == self.IDLE:
            ...
            if distance > PINCH_RELEASE_THRESHOLD:
                # Released quickly → plain click
                pg.click()
                self.state = self.IDLE
                self.pinch_start_time = None
            elif now - self.pinch_start_time >= DRAG_HOLD_DURATION:
                # Held long enough → begin drag
                pg.mouseDown()
                self.state = self.DRAGGING
                self.drag_anchor_x = mid_x
                self.drag_anchor_y = mid_y
            ...
            if distance > PINCH_RELEASE_THRESHOLD:
                ...
                # Steer cursor via midpoint of pinched fingers (more stable
                # than index tip when fingers are touching).
                self._move_drag_cursor(mid_x, mid_y)
    ...
    def _move_drag_cursor(self, mid_x: float, mid_y: float) -> None:
        """During drag, move cursor using the pinch midpoint (stable) with the
        same exponential smoothing used for normal cursor movement."""
        global _cursor_x, _cursor_y
        target_x = min((1 - mid_x) * SCREEN_WIDTH, SCREEN_WIDTH - 1)
        target_y = min(mid_y * SCREEN_HEIGHT, SCREEN_HEIGHT - 1)
        if _cursor_x is None:
            _cursor_x, _cursor_y = target_x, target_y
        else:
            _cursor_x += (target_x - _cursor_x) * SMOOTHING
            ...
            if results.hand_landmarks and results.handedness:
                ...
                for hand_landmarks, handedness in zip(results.hand_landmarks, results.handedness):
                    ...
                    if hand_label == "right":
                        ...
                        # Freeze cursor position while left hand is dragging
                        move_cursor(index_tip.x, index_tip.y, locked=drag_state.is_dragging)
