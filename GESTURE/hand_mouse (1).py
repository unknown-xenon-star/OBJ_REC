...
class ClickDragState:
    ...
    def update(self, index_tip, thumb_tip) -> None:
        ...
        # Blend tracking point: as fingers close, smoothly shift from index tip
        # toward the midpoint (more stable when fingers are touching).
        pinch_ratio = 1.0 - min(distance / PINCH_RELEASE_THRESHOLD, 1.0)
        track_x = index_tip.x + (((index_tip.x + thumb_tip.x) / 2) - index_tip.x) * pinch_ratio
        track_y = index_tip.y + (((index_tip.y + thumb_tip.y) / 2) - index_tip.y) * pinch_ratio
        now = time.monotonic()
        ...
        if self.state == self.IDLE:
            ...
            if distance > PINCH_RELEASE_THRESHOLD:
                ...
                # mouseDown fires at the *current* smoothed cursor position
                # (already there from prior frames), so Paint sees no jump.
                pg.mouseDown()
            ...
            if distance > PINCH_RELEASE_THRESHOLD:
                ...
                # Drive cursor through the unified smoothing pipeline —
                # guarantees continuity with the pre-drag path.
                _smooth_move(track_x, track_y)
